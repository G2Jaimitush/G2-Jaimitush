#include <string.h>
 ...
 const int MaximoProductos = 10;
 const int MaximoNombre = 30;

// Funciones
int pedirCantidad();
float totals(float precios[],int cantidad);
...
void editarProducto(char nombres[][MaximoNombre], float precios[], int cantidad) {
    char nombreBusqueda[MaximoNombre];
    printf("\nIngrese el nombre del producto a buscar: ");
    fflush(stdin);
    fgets(nombreBusqueda, MaximoNombre, stdin);
...
void eliminarProducto(char nombres[][MaximoNombre], float precios[], int cantidad) {
    char nombreBusqueda[MaximoNombre];
    printf("\nIngrese el nombre del producto a buscar: ");
    fflush(stdin);
    fgets(nombreBusqueda, MaximoNombre, stdin);
    ...
    for (int i = 0; i < cantidad; i++) {
        if (strcmp(nombreBusqueda, nombres[i]) == 0) {
            printf("Producto encontrado: %s - $%.2f\n", nombres[i], precios[i]);
            encontrado = 1;
            //eliminar producto de posición y recorrer todas
            nombres[i][strcspn(nombres[i], nombreBusqueda)] = '\0';
            precios[i]=0;
            break;
        }
    }
